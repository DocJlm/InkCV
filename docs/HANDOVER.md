...
当前基线为 **101 个单元/属性测试全绿**，typecheck 零错误，`pnpm --filter @inkcv/web build` 通过；
...
v0.3 在不提升 schema 的前提下补齐：界面语言/简历语言解耦、正文自动语言识别、严格校验的整份
简历 AI 翻译副本、适合宽度/真实 100%/50%–200%/全屏 PDF 预览、可持久化双分隔线、响应式
样式抽屉，以及四款精选页面组合和 8 张中英文 PDF 缩略图。已删除模板 ID 由 renderer 安全回退到 `onyx`。
...
- Markdown 整体替换为"手写风格"文本（`# 姓名`、散文段、无注册表的 `##`）→ 宽容解析成功，
  设置保留、ID 稳定
- 4 款 PDF 模板（玄墨 / 青石 / 极简 ATS / 技术密排）、真实缩略图画廊
- 界面语言不修改简历；排版语言默认自动识别，可在样式高级设置覆盖，AI 翻译始终创建副本
- PDF 默认适宽，100% 为真实 CSS 尺寸，支持 50%–200% 和全屏
- 响应式样式抽屉（黑/蓝预设、自定义颜色、字号/行高/间距/页边距、排版语言覆盖）
- ≥1280px 双分隔线三栏；900-1279px 列表抽屉和编辑/预览分隔线；<900px 编辑/预览双标签工作台
...
packages/core        schema(zod) + Markdown 方言引擎 + reconciliation ← 整个项目的地基
  ...
  src/language.ts                正文语言自动识别与 localeMode 解析
...
packages/renderer    react-pdf 模板 + 字体 + 预览管线
  ...
  src/templates/                 4 款模板 + 共享分页/文本/照片原语
...
- core：`detectResumeLocale(doc) / resolveResumeLocale(doc)` 是所有排版语言判断的唯一入口
