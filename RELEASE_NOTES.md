# InkCV v0.3.0 Public Beta / 公测版

[Web app / 在线版](https://inkcv.vercel.app) · [Privacy / 隐私说明](https://github.com/DocJlm/InkCV/blob/v0.3.0/docs/PRIVACY.md) · [Discussions / 反馈](https://github.com/DocJlm/InkCV/discussions)

## 中文

InkCV v0.3 聚焦语言表达、PDF 预览和模板差异，同时保留 v0.2 的 Web、手机与三平台桌面闭环。

### 新功能

- 表单与 Markdown 双向编辑，预览和导出共用同一条 PDF 编译路径。
- 八款独立中英文布局：玄墨、青石、经典、极简 ATS、技术密排、侧标、脉络与名片。
- 明确区分界面语言与简历语言；AI 可翻译全部内容并创建严格校验的新副本，永不覆盖原文。
- PDF 默认适合宽度，100% 使用真实 96 CSS DPI，并支持 50%–200% 与全屏高清重绘。
...
简历留在当前设备；没有账号、数据库、云同步或公共 AI 额度。Web Key 只在当前页面内存中，桌面 Key 优先存入系统凭据库。数据导出和备份不包含 Key。Minimal ATS 是机器可读性友好设计，不代表任何 ATS 认证。v0.3 不含代码签名、公证、应用商店、自动更新或 DOCX 导出。

## English

InkCV v0.3 focuses on language clarity, PDF preview fidelity, and genuinely distinct templates while preserving the v0.2 cross-platform workflow.

### What's new

- Bidirectional form/Markdown editing with one shared PDF preview and export pipeline.
- Eight distinct bilingual layouts: Onyx, Lapis, Classic, Minimal ATS, Compact Tech, Section Rail, Timeline, and Profile.
- Independent interface/résumé languages and strictly validated AI translation copies that never overwrite the source.
- Fit-width preview, true 96 CSS DPI at 100%, 50%–200% zoom, and high-resolution full screen.
